...
const doStep = function(start, step){
    return start.then(function(result){
        console.log(result[step]);
        const r = result[step].call();
        console.log(r);
        return r;
    })
};

const pipe = function(...steps){
...
const diffIndex = function (repo, commit, resource, cb) {
...
const diffWorkdir = function (repo, commit, resource, cb) {
    return commit.getTree()
        .then(function (tree) {
