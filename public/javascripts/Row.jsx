			...
			render() {
				return (
					<div className="entry" onClick={this.location}>
						...
						<div id="block2" className="entry-cell diff-area diff">{diff(this.props.properties)}</div>
						<div id="block3" className="entry-cell entry-area cache">{isInCache(this.props.properties) ? this.props.name : ''}</div>
						<div id="block4" className="entry-cell diff-area diffcached">{diffCached(this.props.properties)}</div>
